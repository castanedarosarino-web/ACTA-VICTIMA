...
class ActaPDF(FPDF):
    ...
    def crear_cuadros(self, d, relato):
        ...
        self.set_font('Arial', '', 9)
        self.cell(30, 8, ' NOMBRE:', 1)
        self.cell(160, 8, d['nombre'], 1, 1)
        
        self.cell(30, 8, ' DNI:', 1)
        self.cell(65, 8, d['dni'], 1)
        self.cell(30, 8, ' EDAD / SEXO:', 1)
        self.cell(65, 8, f"{d['edad']} AÑOS / {d['sexo']}", 1, 1)
        
        self.cell(30, 8, ' DOMICILIO:', 1)
        self.cell(160, 8, d['domicilio'], 1, 1)
        
        self.cell(30, 8, ' NACIONALIDAD:', 1)
        self.cell(65, 8, d['nacionalidad'], 1)
        self.cell(30, 8, ' EST. CIVIL:', 1)
        self.cell(65, 8, d['est_civil'], 1, 1)

        self.cell(30, 8, ' OCUPACIÓN:', 1)
        self.cell(160, 8, d['ocupacion'], 1, 1)

        self.cell(30, 8, ' CONTACTO:', 1)
        self.cell(160, 8, f"CEL: {d['tel']} - EMAIL: {d['email']}", 1, 1)
...
if relato_crudo:
    ...
    if protocolos:
        ...
        temor = col_q.checkbox("¿Manifiesta Temor por su integridad?", value=True) if "AMENAZAS CALIFICADAS" in protocolos else False
        ...
        # SECCIÓN 3: Doble Panel de Trabajo con IA
        col_in, col_out = st.columns(2)
        
        with col_in:
            st.info("📂 **PASO 1: REVISIÓN POR IA**")
            st.caption("Copie este texto manualmente y procéselo en la IA:")
        ...
        with col_out:
            ...
            st.caption("Pegue aquí el resultado 'cocinado' por la IA:")
            acta_final = st.text_area("Texto final procesado:", height=250, placeholder="Pegue aquí el texto revisado...")
            
            if acta_final:
                # SECCIÓN 4: Exportación a PDF (Punto culminante)
                if st.button("🖨️ GENERAR ACTA EN PDF"):
                    d_pdf = {
                    ...
                    with open(output_name, "rb") as f:
                        st.download_button("⬇️ DESCARGAR ACTA LISTA PARA IMPRIMIR", f, file_name=output_name)

# Control de errores en barra lateral
if not (nombre and dni and lugar):
    st.sidebar.error("❌ Faltan datos obligatorios (Nombre, DNI o Lugar)")
else:
    st.sidebar.success("✅ Sistema listo para procesar")
